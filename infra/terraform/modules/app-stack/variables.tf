...
#              nothing to firewall. The caller creates the tunnel and passes
#              its connector token; see ../cloudflare-tunnel.
#
...
variable "tunnel_token" {
  description = "Connector token for ingress = \"tunnel\". Generated by Cloudflare rather than chosen, so the caller reads it from a data source instead of asking a human for it."
  type        = string
  default     = ""
  sensitive   = true
}
